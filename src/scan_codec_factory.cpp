...
/// </summary>
[[nodiscard]] frame_info update_component_count(const frame_info& frame_info, const coding_parameters& parameters) noexcept
{
    return {frame_info.width, frame_info.height, frame_info.bits_per_sample,
            parameters.interleave_mode == interleave_mode::none ? 1 : frame_info.component_count};
}
...
unique_ptr<ScanProcess> make_codec(const Traits& traits, const frame_info& frame_info, const coding_parameters& parameters)
{
    ...
    {
        return make_unique<scan_encoder_impl<Traits>>(traits, update_component_count(frame_info, parameters), parameters);
    }
    else
    {
        return make_unique<scan_decoder_impl<Traits>>(traits, update_component_count(frame_info, parameters), parameters);
    }
}
...
unique_ptr<ScanProcess> scan_codec_factory<ScanProcess>::create_codec(const frame_info& frame_info,
                                                                      ...
                                                                      const jpegls_pc_parameters& preset_coding_parameters)
{
    ...
    {
        codec = try_create_optimized_codec(frame_info, parameters);
    }
    ...
    if (!codec)
    {
        ...
        {
            default_traits<uint8_t, uint8_t> traits(calculate_maximum_sample_value(frame_info.bits_per_sample),
            ...
            codec = make_codec<ScanProcess, default_traits<uint8_t, uint8_t>>(traits, frame_info, parameters);
        }
        else
        {
            default_traits<uint16_t, uint16_t> traits(calculate_maximum_sample_value(frame_info.bits_per_sample),
                                                      parameters.near_lossless, preset_coding_parameters.reset_value);
            traits.maximum_sample_value = preset_coding_parameters.maximum_sample_value;
            codec = make_codec<ScanProcess, default_traits<uint16_t, uint16_t>>(traits, frame_info, parameters);
        }
    }
